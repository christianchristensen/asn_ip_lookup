...
sqlite3 -header asn_ip.db "SELECT r.cidr,r.asN AS asn,r.dec_width AS ip_width,a.name FROM routeviews_ipv4 r JOIN asn_names a ON r.asN=a.asn WHERE ${IPD}>=r.dec_start AND ${IPD}<=r.dec_end GROUP BY r.asN,r.dec_width ORDER BY r.dec_width ASC LIMIT 10;"
